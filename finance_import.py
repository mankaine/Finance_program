...
# User interface module that allows the user to import a .txt file containing information about transactions and record them 
# into the collections of transactions.

# When a user chooses the option to import transactions from the main_menu, command is given to handle_import_choice. It prompts the 
# user to enter a .txt file location to import, and prompts so until a valid one is provided. The user will enter information as to 
# whether to edit, and will be shown the transactions to confirm. The user can then import another file, repeating the procedure, or
...
def handle_import_choice(
    ...
    while importing:
        ...
        if file_name == basic_view.KILL_PHRASE:
            ...
            year, month_name, temp_inflows, temp_outflows = _import_transxs(    # temp_inflows, temp_outflows imported from .txt file
            file_name)                                                          # Added to CashFlows objects when all edits complete
            
            finance_view.view_transxs(                                          # View imported transactions
            ...
            update_dicts(inflows, temp_inflows)                                 # Integrate temp_flows into permanent flows
...
def update_dicts (                                                              # Preferable to dict1.update(dict2) - saves both 
    orig_cfs: cashflow.CashFlows, temp_cfs: cashflow.CashFlows) -> None:        # dict's values. Called after all transactions edited
...
def _valid_file_type(extension: str, file_name: str) -> bool:
    ...
    '''
    return file_name.lower().endswith(extension)                                # Passed to file_to_import
...
def _import_transxs(file_name: str) -> (int, str, cashflow.CashFlows, 
    ...
    try:
        ...
        with io.open(file_name, "r", encoding="utf-8") as file_imported:        # Note utf-8 encoding
            ...
            try:
                ...
                for line in imported_lines: 
                    if _contains_reject_condition(line):
                        continue
                    else:                                                       # Line contains transaction
                        new_cf = _to_CF_from(line, month_name, year)
                        if line[76] == "+":                                     # inflow
                            temp_inflows = handle_new_cf(
                                            inflow_list, new_cf, temp_inflows)
                        elif line[76] == "-":
                            temp_outflows = handle_new_cf(                      # outflow
                                        outflow_list, new_cf, temp_outflows)                            
                return year, month_name, temp_inflows, temp_outflows            # passed ot main function
            except Exception as e:
                print(
                "An error has occurred while reading the file: {}".format(e))
            finally:                                                            # Prevents leakage
                file_imported.close()
    except Exception as e:
        print("An error occurred while opening the file: {}".format(e))
    finally:
        basic_view.print_loading_newline("File Closed")                         # Success sign for closed file
    

def _contains_reject_condition(line: str) -> bool:                              # Return value used to determine how to process line
    '''Returns True if the line contains a condition that marks it as not 
    a transaction. False otherwise.
    '''                                                                         # Passed to _import_transxs
    return "Transactions" in line \
            ...
            or '\n' == line \
            or line.strip() == ""                                               # Any conditions met => ignore line when importing
...
def _to_CF_from(
    ...
    try:
        ...
        pass                                                                    # Error => overlook newly constructed object entirely
...
def _convert_str_to_curr(cf: cashflow.CashFlow, read_line: str) -> None:        # Price Conversion
...
def _convert_str_to_cf_dirx(cf: cashflow.CashFlow, read_line: str) -> None:     # Cash Flow Conversion
